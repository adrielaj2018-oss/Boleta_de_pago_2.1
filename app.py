...
def transporte():
    ...
    vencidos = scalar("""SELECT COUNT(*) AS c FROM transporte_conductores
                               WHERE (venc_licencia IS NOT NULL AND venc_licencia<>'' AND venc_licencia<=?)
                                  ...
                                  OR (venc_sctr IS NOT NULL AND venc_sctr<>'' AND venc_sctr<=?)""", (hoy,hoy,hoy)) + scalar("""SELECT COUNT(*) AS c FROM transporte_vehiculos
    ...
    body = """
    <style>
      .shell{max-width:430px!important;padding:4px 6px!important;margin:0 auto!important;background:#fff!important;overflow-x:hidden!important}.phone-wrap{width:100%!important;max-width:390px!important;margin:0 auto!important}.page-card{width:100%!important;max-width:390px!important;margin:0 auto!important;border-radius:12px!important;overflow:hidden!important;border:1px solid #e5e7eb!important;box-shadow:0 10px 26px rgba(0,0,0,.10)!important}.tm-head{height:132px;background:linear-gradient(135deg,#075d2a,#2f773b);color:#fff;text-align:center;position:relative;padding-top:18px;border-radius:12px 12px 0 0}.tm-back{position:absolute;left:16px;top:18px;color:white;text-decoration:none;font-size:32px;line-height:1}.tm-config{position:absolute;right:13px;top:18px;color:#fff;text-decoration:none;border:1px solid rgba(255,255,255,.65);border-radius:9px;padding:7px 9px;font-size:11px;font-weight:900;display:flex;gap:4px;align-items:center}.tm-config i{font-size:14px}.tm-bus{font-size:38px;line-height:1;margin-bottom:7px}.tm-title{font-size:14px;font-weight:900;letter-spacing:.3px}.tm-card{background:#fff;margin:-32px 12px 12px;border-radius:12px;padding:14px 12px;position:relative;z-index:3;box-shadow:0 7px 18px rgba(0,0,0,.12)}.tm-top-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:12px;margin-bottom:16px}.tm-tile{background:linear-gradient(135deg,#0b632e,#2f773b);color:#fff;text-decoration:none;border-radius:9px;min-height:78px;display:flex;align-items:center;justify-content:center;flex-direction:column;font-weight:900;font-size:12px;box-shadow:0 6px 15px rgba(0,0,0,.14);text-align:center}.tm-tile i{font-size:25px;margin-bottom:6px}.tm-section-title{font-size:11px;font-weight:900;color:#065f2a;text-transform:uppercase;margin:13px 0 8px}.tm-sub-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:11px}.tm-sub-grid.two{grid-template-columns:repeat(2,1fr)}.tm-mini{background:linear-gradient(135deg,#0b632e,#2f773b);color:white;text-decoration:none;border-radius:8px;min-height:78px;padding:8px 4px;display:flex;align-items:center;justify-content:center;flex-direction:column;text-align:center;font-weight:900;font-size:10.5px;line-height:1.12;box-shadow:0 5px 13px rgba(0,0,0,.12)}.tm-mini i{font-size:23px;margin-bottom:6px}.tm-kpis{display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin-top:14px}.tm-kpi{border:1px solid #b7dcc0;background:#f8fff9;border-radius:7px;text-align:center;padding:7px 3px;color:#065f2a;font-weight:900;font-size:9px;min-height:58px;display:flex;flex-direction:column;justify-content:center}.tm-kpi strong{font-size:18px;line-height:1.1}.tm-foot{font-size:10px;color:#6b7280;text-align:center;margin:12px 0 2px}@media(max-width:420px){.phone-wrap,.page-card{max-width:370px!important}.tm-card{margin-left:10px;margin-right:10px}.tm-top-grid{gap:10px}.tm-tile{min-height:74px}.tm-sub-grid{gap:9px}.tm-mini{min-height:74px;font-size:10px}}
    </style>
    <div class="phone-wrap desktop-pad"><div class="page-card">
      <div class="tm-head">
        ...
        <div class="tm-bus"><i class="bi bi-bus-front"></i></div>
        <div class="tm-title">MÓDULO TRANSPORTE</div>
      </div>
      <div class="tm-card">
        <div class="tm-top-grid">
          <a class="tm-tile" href="{{url_for('transporte_conductores')}}"><i class="bi bi-person-vcard"></i>Conductores</a>
          <a class="tm-tile" href="{{url_for('transporte_vehiculos')}}"><i class="bi bi-bus-front"></i>Buses</a>
          <a class="tm-tile" href="{{url_for('transporte_rutas')}}"><i class="bi bi-geo-alt"></i>Rutas</a>
        </div>
        <div class="tm-section-title">Operación</div>
        <div class="tm-sub-grid">
          <a class="tm-mini" href="{{url_for('transporte_rutas')}}"><i class="bi bi-calendar-check"></i>Rutas de hoy</a>
          <a class="tm-mini" href="{{url_for('transporte_abordaje')}}"><i class="bi bi-qr-code-scan"></i>Abordaje<br>trabajadores</a>
          <a class="tm-mini" href="{{url_for('transporte_mapa_general')}}"><i class="bi bi-geo-alt"></i>GPS /<br>seguimiento</a>
          <a class="tm-mini" href="{{url_for('conductor_movil_login')}}"><i class="bi bi-phone"></i>Móvil<br>conductor</a>
        </div>
        <div class="tm-section-title">Maestros</div>
        <div class="tm-sub-grid two">
          <a class="tm-mini" href="{{url_for('transporte_carga_masiva')}}"><i class="bi bi-upload"></i>Carga masiva</a>
          <a class="tm-mini" href="{{url_for('transporte_pines')}}"><i class="bi bi-key"></i>PIN conductor</a>
        </div>
        <div class="tm-section-title">Control</div>
        <div class="tm-sub-grid two">
          <a class="tm-mini" href="{{url_for('exportar_transporte_pasajeros')}}"><i class="bi bi-file-earmark-excel"></i>Reporte<br>abordajes</a>
          <a class="tm-mini" href="{{url_for('transporte_requisitos')}}"><i class="bi bi-exclamation-triangle"></i>Requisitos<br>vencidos</a>
        </div>
        <div class="tm-kpis">
          <div class="tm-kpi">Rutas hoy<strong>{{rutas_hoy}}</strong></div>
          <div class="tm-kpi">Abordaron hoy<strong>{{abordajes}}</strong></div>
          <div class="tm-kpi">No subieron<strong>{{no_subieron}}</strong></div>
          <div class="tm-kpi">Conductores<strong>{{conductores}}</strong></div>
          <div class="tm-kpi">Buses<strong>{{vehiculos}}</strong></div>
          <div class="tm-kpi">Vencidos<strong>{{vencidos}}</strong></div>
        </div>
        <div class="tm-foot">Seleccione un submódulo para ver o gestionar la información.</div>
      </div>
    </div></div>
    """
    return render_page(body, conductores=conductores, vehiculos=vehiculos, rutas_hoy=rutas_hoy, abordajes=abordajes, no_subieron=no_subieron, vencidos=vencidos)

@app.route('/transporte/config')
@login_required
def transporte_config():
    body = """
    <style>.shell{max-width:520px!important}.phone-wrap,.page-card{max-width:500px!important}.cfg-head{background:linear-gradient(135deg,#075d2a,#2f773b);color:#fff;padding:14px 16px;border-radius:12px 12px 0 0;display:flex;align-items:center;gap:10px;font-weight:900;font-size:18px}.cfg-list{background:#fff;border:1px solid #e5e7eb;border-top:0;border-radius:0 0 12px 12px;box-shadow:0 6px 18px rgba(0,0,0,.08)}.cfg-item{display:flex;align-items:center;gap:14px;padding:15px 18px;border-bottom:1px solid #edf0ed;color:#1f2937;text-decoration:none;font-weight:800}.cfg-item:last-child{border-bottom:0}.cfg-item i{font-size:23px;color:#08713b}.cfg-item .chev{margin-left:auto;color:#111}</style>
    <div class="phone-wrap desktop-pad"><div class="page-card"><div class="cfg-head"><a class="text-white text-decoration-none" href="{{url_for('transporte')}}"><i class="bi bi-chevron-left"></i></a><i class="bi bi-gear"></i> CONFIGURACIONES</div><div class="cfg-list">
      ...
      <a class="cfg-item" href="{{url_for('transporte_conductores')}}"><i class="bi bi-phone"></i>Relación conductor - móvil <i class="bi bi-chevron-right chev"></i></a>
