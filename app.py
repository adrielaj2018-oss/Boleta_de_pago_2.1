...
def login():
    ...
    body=f'''<div class="login-wrap login-pro"><div class="login-green"><div class="mini-top"><button type="button" onclick="fakeOk('Soporte habilitado')">{app_icon('support')} Soporte</button><button type="button" onclick="fakeOk('Configuración disponible luego del ingreso')">{app_icon('settings')} Config.</button></div><div class="login-mark"><div>P<span>&</span>A</div></div><h1>P&A Mobile</h1><p>Gestión del Capital Humano</p></div><form class="login-card login-float" method="post"><h2>Inicia sesión para continuar</h2>{f'<div class="err">{err}</div>' if err else ''}<div class="label">Usuario</div><input class="input" name="usuario" value="11223344" maxlength="8" inputmode="numeric" required><div class="label">Empresa</div><select class="input"><option>{COMPANY}</option></select><div class="label">Contraseña</div><input class="input" name="password" value="123456" type="password" required><button class="login-btn">{app_icon('logout')} Iniciar Sesión</button><div class="link">Olvidaste tu contraseña?</div></form><div class="help">¿Problemas para acceder?<br><b>Contactar a Mesa</b></div></div>'''
...
def documentos():
    ...
    rows=''.join([f'<a class="doc-row" href="/documentos/{k}"><div class="doc-ico">{app_icon('download')}</div><div><div class="doc-title">{t}</div><div class="doc-sub">Disponible para descarga</div></div><div class="chev">›</div></a>' for k,t in docs])
    body=head("Gestión de documentos","docs")+f'<div class="doc-list">{rows}</div>{bottom("documentos")}'
