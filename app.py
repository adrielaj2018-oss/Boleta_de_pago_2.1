...
async function marcar(tipo){let btn=document.querySelector('[data-tipo="'+tipo+'"]'); if(btn)btn.classList.add('active'); let p=document.getElementById('pin')||{}; let payload={tipo:tipo,hora_manual:(document.getElementById('selectedTime')?.dataset.value||''),lat:p.dataset?.lat||'',lng:p.dataset?.lng||'',precision:p.dataset?.acc||''}; function send(){fetch('/api/marcar',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)}).then(r=>r.json()).then(j=>{if(j.ok){if(btn){btn.classList.add('done');btn.innerHTML='✓ '+btn.textContent.replace('✓ ','')}showToast(j.msg||'Registrado');setTimeout(()=>location.reload(),900)}else showToast(j.msg||'Error',false)}).catch(()=>showToast('No se pudo registrar',false));} if(navigator.geolocation){navigator.geolocation.getCurrentPosition(pos=>{payload.lat=pos.coords.latitude;payload.lng=pos.coords.longitude;payload.precision=Math.round(pos.coords.accuracy);send();},()=>send(),{enableHighAccuracy:true,timeout:6000});}else send();}
